...
### JWT
JWT (JSON Web Token) authentication is stateless and does not have an entry point. The `auth(payload, callback)` function must verify `payload.sub`, and mustmust execute `callback(err, user)`.
