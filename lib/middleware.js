...
function stream (req, res, next) {
	if (req.file === void 0) {
		...
		const headers = {
			...
			"last-modified": req.file.mtime.toUTCString()
		};
		...
		if (req.server.canETag(req.parsed.pathname, req.method) === true) {
			letag = headers.etag = req.server.etag(req.parsed.pathname, req.file.size, req.file.mtime);
		}
		...
		if (regex.get_only.test(req.method) === true) {
			if (letag !== void 0 && req.headers["if-none-match"] === letag) {
			...
			} else if (req.headers["if-none-match"] === void 0 && Date.parse(req.headers["if-modified-since"]) >= req.file.mtime) {
				delete headers["content-length"];
				res.send("", 304, headers);
			} else {
				options = {};
				...
				if (req.headers.range !== void 0) {
					...
					if (isNaN(options.start) === true && isNaN(options.end) === false) {
						options.start = req.file.size - options.end;
						options.end = req.file.size;
					...
					status = 206;
					headers["content-range"] = `bytes ${options.start}-${options.end}/${req.file.size}`;
