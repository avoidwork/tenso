...
function bypass (req, res, next) {
	req.unprotect = (req.cors === true && req.method === "OPTIONS") || req.server.config.auth.unprotect.filter(i => i.test(req.url) === true).length > 0; // eslint-disable-line no-extra-parens
	next();
}
...
function guard (req, res, next) {
	...
	if (req.parsed.pathname === login || req.isAuthenticated() === true) {
		next();
	} else {
...
function parse (req, res, next) {
	...
	if (req.body !== "") {
		...
		} else if (type.includes("application/json") === true || jsonWrap(req.body) === true) {
			try {
...
function keymaster (req, res) {
	...
	if (req.protect === false || req.protectAsync === false || (req.session !== void 0 && req.isAuthenticated() === true) === true) {
		...
		if (routes.has(uri)) {
			...
			if (result !== void 0) {
				req.server[req.method.toLowerCase()](req.parsed.pathname, result); // Memoizing the route
