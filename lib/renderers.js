...
renderers.set("text/csv", (arg, req, headers) => {
	...
	headers["content-disposition"] = String.raw`attachment; filename="${(req.parsed.pathname.replace(/.*\//g, "") || "download").replace(/\..*/, "_")}.csv"`;
...
renderers.set("text/html", (arg, req, headers, tpl) => {
	...
	return (tpl || "")
		...
		.replace("{{headers}}", Reflect.ownKeys(headers).sort().map(i => String.raw`<tr><td>${i}</td><td>${sanitize(headers[i])}</td></tr>`).join("\n"))
		.replace("{{formats}}", String.raw`<option value=''></option>${(Array.from(renderers.keys()).filter(i => i.indexOf("html") === -1).map(i => "<option value='" + i + "'>" + i.replace(/^.*\//, "").toUpperCase() + "</option>").join("\n"))}`)
		...
		.replace("{{methods}}", utility.explode(headers.allow.replace("GET, HEAD, OPTIONS", "")).filter(i => i !== "").map(i => String.raw`<option value='${i}'>${i}</option>`).join("\n"))
...
renderers.set("application/javascript", (arg, req, headers) => {
	...
	return String.raw`${(req.parsed.query.callback || "callback")}(${JSON.stringify(arg, null, 0)});`;
});
