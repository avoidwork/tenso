...
function text (req, res, arg) {
	return Array.isArray(arg) ? arg.map(i => text(req, res, i)).join(",") : arg instanceof Object ? JSON.stringify(arg) : arg.toString();
}

renderers.set("application/json", (req, res, arg) => JSON.stringify(arg, null, indent(req.headers.accept, req.server.config.json || 0)));
renderers.set("application/yaml", (req, res, arg) => yaml.stringify(arg, indent(req.headers.accept, req.server.config.json || 0)));
