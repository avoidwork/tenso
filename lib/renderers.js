...
function text (arg) {
	return Array.isArray(arg) ? arg.map(i => text(i)).join(",") : arg instanceof Object ? JSON.stringify(arg) : arg.toString();
}
...
renderers.set("application/json", (arg, req) => JSON.stringify(arg, null, indent(req.headers.accept, req.server.config.json || 0)));
renderers.set("application/yaml", (arg, req) => yaml.stringify(arg, indent(req.headers.accept, req.server.config.json || 0)));
