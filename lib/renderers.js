...
renderers.set("text/html", function (arg, req, headers, tpl) {
	...
	}).join("\n")).replace("{{formats}}", "<option value=''></option>" + Array.from(renderers.keys()).filter(function (i) {
		return i.indexOf("html") === -1;
	}).map(function (i) {
		return "<option value='" + i + "'>" + i.replace(/^.*\//, "").toUpperCase() + "</option>";
	}).join("\n")).replace("{{body}}", JSON.stringify(arg, null, 2)).replace("{{year}}", new Date().getFullYear()).replace("{{version}}", "3.3.11").replace("{{allow}}", headers.allow).replace("{{methods}}", utility.explode(headers.allow.replace("GET, HEAD, OPTIONS", "")).filter(function (i) {
		return i !== "";
	}).map(function (i) {
