	...
	sort (arg, req) {
		...
		if (typeof req.parsed.search === "string" && regex.has_order_by.test(req.parsed.search) === true && Array.isArray(arg)) {
			if (regex.unsortable.test(typeof arg[0]) === false && arg[0] !== null) {
				output = keysort(utility.clone(arg), req.parsed.search.replace("?", "").split("&").filter(i => regex.order_by.test(i)).reduce((a, b) => [...a, b.replace(regex.order_by, "")], []).join(", ").replace(/\%20|\+/g, " "));
			} else {
