...
class Tenso extends Base {
	...
	error (req, res, err, headers) {
		...
		res.send(preset ? err : new Error(http.STATUS_CODES[status]), status, headers);
	}
	...
	headers (req, res, status, body, headers) {
		...
		if (req.cors) {
			headers["access-control-expose-headers"] = `cache-control, content-language, content-type, expires, last-modified, pragma${csrf ? `, ${this.config.security.key}` : ""}${this.config.corsExpose.length > 0 ? `, ${this.config.corsExpose}` : ""}`;
		}
		...
		if (canGet(req.method) === false || status >= 400) {
			delete headers.etag;
			delete headers["last-modified"];
		}
	...
	render (req, res, arg) {
		...
		const accepts = explode(res.getHeader("content-type") || req.parsed.searchParams.get("format") || req.headers.accept, ",");
		let format = "",
			renderer, result;
		...
		switch (status) {
			...
			default:
				...
				if (nil || typeof body.on !== "function") {
					headers["content-length"] = nil ? 0 : Buffer.byteLength(body);
