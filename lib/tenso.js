...
const path = require("path"),
	...
	HTTPMethods = require(path.join(__dirname, "httpmethods.js"));
	...
	send (req, res, body = "", status = 200, headers = {"content-type": "text/plain"}) {
		return new Promise((resolve, reject) => {
			if (res.headersSent === false) {
				...
				output = body === null || typeof body.on !== "function" ? this.render(req, res, utility.hypermedia(this, req, this.serialize(req, body, status), headers), headers) : body;
				...
				const head = regex.head.test(req.method),
					pipe = head === false && output !== null && typeof output.on === "function",
				...
				if (res.headersSent === false) {
					...
					if (pipe === false && output instanceof Object || Array.isArray(output)) {
					...
					this.headers(req, res, status, output, headers, pipe);
					...
					if (status !== 416 && req.headers.range !== void 0 && !res.hasHeader("content-range") && headers["content-range"] === void 0) {
						...
						if (head === false && output !== null && output !== "") {
						...
						if (compression !== "") {
							...
							if (pipe === true && this.config.http2 === false) {
								...
								output.pipe(zlib[compressionMethod]()).on("error", e => utility.error(req, res, this, reject, e, headers || rheaders)).on("close", () => resolve(true)).pipe(res);
							} else {
								zlib[compressionMethod.replace("create", "").toLowerCase()](output, (e, data) => {
									if (e !== null) {
							...
							if (this.config.http2 === false) {
								...
								if (pipe === true) {
									output.on("error", e => utility.error(req, res, this, reject, e, headers || rheaders)).on("close", () => resolve(true)).pipe(res);
								} else {
									if (req.range !== void 0) {
										res.end(Buffer.from(output.toString()).slice(req.range.start, req.range.end + 1).toString());
									} else if (output === null) {
										res.end("null");
									} else {
								...
								this.router.http2Send(req, res, pipe === true ? output : req.range !== void 0 ? Buffer.from(output.toString()).slice(req.range.start, req.range.end + 1).toString() : output.toString(), status, headers);
							}
						}
					}
				}
