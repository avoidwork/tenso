...
class Tenso {
	...
	sort (arg, req) {
		...
		if (typeof req.parsed.search === "string") {
			order_by = req.parsed.search.replace("?", "").split("&").filter(i => orderBy.test(i)).reduce((a, b) => {
			...
			if (order_by !== "") {
				output = keysort(utility.clone(arg), order_by);
			} else {
				output = arg;
			}
		} else {
