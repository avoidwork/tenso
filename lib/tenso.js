...
class Tenso {
	...
	options (route, fn) {
		this.router.use(route, fn, "OPTIONS");
	...
	patch (route, fn) {
		this.router.use(route, fn, "PATCH");

		return this;
	}

	post (route, fn) {
		this.router.use(route, fn, "POST");

		return this;
	}

	put (route, fn) {
		this.router.use(route, fn, "PUT");
			...
			const errHandler = e => {
				try {
					if (this.config.http2 === false) {
						...
						res.server.router.http2Send(res, http.STATUS_CODES[500], 500, headers || rheaders);
					}
				} catch (err) {
					void 0;
				}
					...
					if (compression !== "") {
						...
						if (pipe === true) {
							if (this.config.http2 === false) {
								...
								this.router.http2Send(res, body.pipe(zlib[compressionMethod]()).on("error", errHandler).on("close", () => resolve(true)), status, headers);
							}
						} else {
							zlib[compressionMethod.replace("create", "").toLowerCase()](body, (e, data) => {
								if (e !== null) {
									...
									if (this.config.http2 === false) {
										...
										this.config.router.http2Send(res, data, status, headers);
									}
						...
						if (this.config.http2 === false) {
							...
							this.router.http2Send(res, output, status, headers);
						}
					}
				}
			}
