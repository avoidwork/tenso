...
class HTTPMethods {
	constructor () {
		...
		retsu.each(http.METHODS, i => {
			this[i.toLowerCase()] = (...args) => this.router.use(...args, i);
		});
	}
}

class Tenso extends HTTPMethods {
	constructor () {
		super();
		this.config = {
			address: "0.0.0.0",
	...
	clf (req, res, headers) {
		...
		return this.config.logging.format.replace("%v", req.headers.host)
			...
			.replace("%r", `${req.method} HTTP/${this.config.httpVersion}`)
	...
	del (route, fn) {
		this.router.use(route, fn, "DELETE");
	...
	render (req, res, arg, headers) {
		...
		retsu.each(Object.keys(decorated), i => {
			if (headers[i] === void 0) {
	...
	respond (req, res, arg = "", status = 200, headers = {}) {
		return new Promise((resolve, reject) => {
			if (res.headersSent === false) {
				...
				output = arg === null || typeof arg.on !== "function" ? this.render(req, res, utility.hypermedia(this, req, this.serialize(req, arg, status), headers), headers) : arg;
				...
				this.send(req, res, output, status, headers).then(resolve, reject);
			} else {
				resolve();
			}
		});
	...
	send (req, res, body = "", status = 200, headers = {"content-type": "text/plain"}) {
		return new Promise((resolve, reject) => {
			...
			if (res.headersSent === false) {
				...
				retsu.each(Object.keys(headers), i => {
				...
				if (status !== 416 && req.headers.range !== void 0 && !res.hasHeader("content-range") && headers["content-range"] === void 0) {
					...
					if (head === false && body !== null && body !== "") {
						compression = this.compression(req.headers["accept-encoding"], headers["content-type"] || rheaders["content-type"]);
					}
					...
					if (compression !== "") {
						...
						if (pipe === true && this.config.http2 === false) {
							headers["transfer-encoding"] = "chunked";
							delete headers["content-length"];
							res.writeHead(status, headers);
							body.pipe(zlib[compressionMethod]()).on("error", e => utility.error(req, res, this, reject, e, headers || rheaders)).on("close", () => resolve(true)).pipe(res);
						} else {
							zlib[compressionMethod.replace("create", "").toLowerCase()](body, (e, data) => {
								if (e !== null) {
									utility.error(req, res, this, reject, e, headers || rheaders);
								} else {
									headers["content-length"] = data.length;

									if (req.headers.range !== void 0) {
										this.partial(req, res, data.length, headers);
									}
									...
									if (this.config.http2 === false) {
										...
										this.router.http2Send(req, res, data, status, headers);
									}
						...
						if (this.config.http2 === false) {
							...
							if (pipe === true) {
								body.on("error", e => utility.error(req, res, this, reject, e, headers || rheaders)).on("close", () => resolve(true)).pipe(res);
							} else {
								if (req.range !== void 0) {
							...
							const output = pipe === true ? body : req.range !== void 0 ? Buffer.from(body.toString()).slice(req.range.start, req.range.end + 1).toString() : body.toString();

							this.router.http2Send(req, res, output, status, headers);
						}
					}
				}
			}
		...
		if (this.server === null) {
			if (this.config.ssl.cert === null && this.config.ssl.pfx === null && this.config.ssl.key === null) {
				...
				this.server = http2.createSecureServer({
					...
					pfx: this.config.ssl.pfx ? fs.readFileSync(this.config.ssl.pfx) : void 0,
					key: this.config.ssl.key ? fs.readFileSync(this.config.ssl.key) : void 0
				}).on("stream", this.router.route).listen(this.config.port, this.config.address, drop);
			}
