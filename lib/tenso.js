...
class Tenso {
	...
	respond (req, res, arg, status = 200, headers = {}) {
		...
		if (!res.headersSent) {
			let ref = [merge(headers, res.getHeaders())],
				output;
			...
			if (!regex.get_rewrite.test(req.method)) {
				...
				output = typeof arg.on !== "function" ? this.render(req, utility.hypermedia(this, req, this.serialize(req, arg, status), ref[0]), ref[0]) : arg;
