...
class Tenso {
	constructor (config = {headers: {}}) {
		...
		config.headers["x-powered-by"] = "node.js/" + process.versions.node.replace(/^v/, "");
	...
	respond (req, res, arg, status = 200, headers = {}) {
		...
		if (res._header === null) {
			...
			if (!regex.get_rewrite.test(req.method)) {
				...
				output = this.render(req, utility.hypermedia(this, req, this.serialize(req, arg, status), ref[0]), ref[0]);

				if (this.canETag(req.parsed.pathname, req.method, ref[0])) {
					ref[0].etag = this.etag(output, req.parsed.href);
				}
			} else {
				output = "";
			}
