...
class Tenso extends Base {
	...
	headers (req, res, status, body, result, head) {
		if (req.protect === true && regex.private.test(result["cache-control"]) === false) {
			result["cache-control"] = (result["cache-control"] || res.getHeader("cache-control") || "").replace(/(private|public)(,\s)?/g, "");
			result["cache-control"] = `private${(result["cache-control"].length > 0 ? ", " : "")}${(result["cache-control"] || "")}`;
		}
		...
		if (regex.modify.test(req.method) === false && regex.modify.test(req.allow) === true && this.config.security.csrf === true && res.locals[this.config.security.key] !== void 0) {
			result[this.config.security.key] = res.locals[this.config.security.key];
		}

		if (status === 200 && this.canETag(req.parsed.pathname, req.method, res.getHeader("cache-control") || result["cache-control"])) {
			result.etag = this.etag(body, req.parsed.href);
		} else {
			delete result.etag;
		...
		utility.each(Object.keys(result), i => {
			if (rheaders[i] !== void 0) {
				if (result[i] !== rheaders[i]) {
					res.removeHeader(i);
				} else {
					delete result[i];
				}
