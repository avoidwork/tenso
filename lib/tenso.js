...
class Tenso {
	...
	error (req, res, err) {
		...
		res.send(preset ? err : new Error(http.STATUS_CODES[status]), status);
	}
		...
		if (!res.headersSent) {
			...
			if (!regex.get_rewrite.test(req.method)) {
				if (req.protect && !regex.private.test(headers["cache-control"])) {
					...
					headers["cache-control"] = "private" + (ref[0]["cache-control"].length > 0 ? ", " : "") + (ref[0]["cache-control"] || "");
				...
				if (this.canETag(req.parsed.pathname, req.method, headers)) {
