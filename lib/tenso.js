 ...
 */
var auth = function auth(obj, config) {
	...
	var asyncFlag = function asyncFlag(req, res, next) {
		req.protectAsync = true;
		next();
	};

	var bypass = function bypass(req, res, next) {
	...
	if (config.auth.basic.enabled) {
		(function () {
			...
			var validate = function validate(arg, cb) {
				if (x[arg]) {
					cb(null, x[arg]);
				} else {
 ...
 */
var bootstrap = function bootstrap(obj, config) {
	var notify = false;

	var mediator = function mediator(req, res, next) {
	...
	var parse = function parse(req, res, next) {
		var args = undefined,
		    type = undefined;
 ...
 */
var error = function error(server, req, res, status, err) {
 ...
 * @return {Object}    Tenso instance
 */
var factory = function factory(arg) {
 ...
 */
var hypermedia = function hypermedia(server, req, rep, headers) {
	var seen = {},
	...
	var parse = function parse(obj, rel, item_collection) {
		rel = rel || "related";
 ...
 * @return {Undefined}     undefined
 */
var keymaster = function keymaster(req, res, next) {
 ...
 */
var prepare = function prepare(arg, error, status) {
...
var sanitize = function sanitize(arg) {
	var output = arg;
 ...
 * @return {Undefined}     undefined
 */
var rate = function rate(obj, req, res, next) {
...
var renderers = {
	...
	html: {
		fn: function fn(arg, req, headers, tpl) {
			...
			}).join("\n")).replace("{{body}}", JSON.stringify(arg, null, 2)).replace("{{year}}", new Date().getFullYear()).replace("{{version}}", "1.6.0").replace("{{allow}}", headers["allow"]).replace("{{methods}}", string.explode(headers["allow"].replace("GET, HEAD, OPTIONS", "")).filter(function (i) {
				return i !== "";
			}).map(function (i) {
 ...
 */
var response = function response(arg, status) {
 ...
 */
var zuul = function zuul(protect) {
