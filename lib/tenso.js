					...
					if (status !== 416 && req.headers.range !== void 0 && !res.hasHeader("content-range") && headers["content-range"] === void 0) {
						...
						if (compression !== "") {
							...
							if (this.config.http2 === false) {
								...
								const x = pipe === true ? body : req.range !== void 0 ? Buffer.from(body.toString()).slice(req.range.start, req.range.end + 1).toString() : body.toString();

								this.router.http2Send(req, res, x, status, headers);
							}
						}
					}
				}
