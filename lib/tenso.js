...
class Tenso {
	...
	partial (req, res, size, headers) {
		...
		retsu.each(req.headers.range.replace(/^bytes=/, "").split(",")[0].split("-"), (i, idx) => {
	...
	render (req, res, arg, headers) {
		...
		return renderer(arg, req, merge(Object.assign({}, res.getHeaders()), headers), format === "text/html" ? this.config.template : void 0);
	}
	...
	respond (req, res, arg, status = 200, headers = {}) {
		...
		if (!res.headersSent) {
			...
			if (status === 200 && this.canETag(req.parsed.pathname, req.method, headers)) {
	...
	send (req, res, body = "", status = 200, headers = {"content-type": "text/plain"}) {
		...
		if (!res.headersSent) {
			...
			if (compression) {
				...
				if (head) {
					body = "";
				} else if (lheaders["content-range"] !== void 0 && lheaders["content-range"] !== "") {
