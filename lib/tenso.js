...
class Tenso {
	...
	headers (req, res, status, body, result, pipe = false) {
		...
		if (pipe === false && req.headers.range && result["content-range"] === void 0) {
	...
	partial (req, res, size, headers) {
		if (regex.partial.test(req.headers.range)) {
			...
			if ((options.start >= options.end || isNaN(options.start) || isNaN(options.start)) === false) {
				req.range = options;
	...
	send (req, res, body = "", status = 200, headers = {"content-type": "text/plain"}) {
		...
		if (res.headersSent === false) {
			...
			if (pipe === false && body instanceof Object || body instanceof Array) {
