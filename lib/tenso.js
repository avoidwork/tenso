...
class Tenso extends Base {
	...
	error (req, res, err, headers) {
		...
		try {
			res.send(preset === true ? err : new Error(http.STATUS_CODES[status]), status, headers);
		} catch (e) {
			void 0;
		}
	...
	headers (req, res, status, body, headers) {
		...
		if (canModify(req.method) === false && canModify(req.allow) === true && this.config.security.csrf === true && res.locals[this.config.security.key] !== void 0) {
