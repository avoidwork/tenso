...
function bootstrap (obj) {
	...
	obj.router.onconnect = (req, res) => {
		...
		req.unprotect = false;
		req.server = obj;
		res.error = (status, body, headers) => res.send(body !== void 0 ? body instanceof Error ? body : new Error(body) : new Error(http.STATUS_CODES[status]), status, headers);
		res.send = (body, status, headers) => obj.send(req, res, body, status, headers);
	};
