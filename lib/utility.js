...
function auth (obj, config) {
	const ssl = config.ssl.cert && config.ssl.key,
		...
		stateless = config.rate.enabled === false && config.security.csrf === false,
		authMap = {},
		authUris = [];
	...
	function init (track = false) {
		if (track === true) {
			passportInit = passport.initialize();
			passportSession = passport.session();
			obj.all(passportInit).blacklist(passportInit);
			obj.all(passportSession).blacklist(passportSession);
		}
	}
	...
	obj.all(middleware.parse).blacklist(middleware.parse);
	...
	if (stateless === true) {
		...
		if (authUris.length > 0) {
			...
			(function () {
				...
				r = r.replace(/\|$/, "") + ")).*$";
				obj.all(r, middleware.guard).blacklist(middleware.guard);
			}());
	...
	if (config.auth.basic.enabled === true) {
		(function () {
			...
			if (async === true || config.auth.local.enabled === true) {
				...
				obj.all(passportAuth).blacklist(passportAuth);
			}
		}());
	}
...
function bootstrap (obj) {
	...
	obj.router.onconnect = (req, res) => {
		...
		res.error = (status, body) => res.send(body !== void 0 ? body instanceof Error ? body : new Error(body) : new Error(http.STATUS_CODES[status]), status);
	...
	obj.all(obj.etags.middleware).blacklist(obj.etags.middleware);
	obj.all(middleware.timer).blacklist(middleware.timer);
	obj.all(middleware.payload).blacklist(middleware.payload);
	...
	// Setting routes
	each(Object.keys(obj.config.routes), k => {
		const method = k,
