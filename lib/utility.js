...
function explode (obj, arg = ",") {
	return trim(obj).split(new RegExp(String.raw`\s*${arg}\s*`));
}
...
function auth (obj, config) {
	const ssl = config.ssl.cert && config.ssl.key,
		realm = String.raw`http${(ssl ? "s" : "")}://${(config.hostname === "localhost" ? "127.0.0.1" : config.hostname)}${(config.port !== 80 && config.port !== 443 ? ":" + config.port : "")}`,
	...
	retsu.each(["protect", "unprotect"], k => {
		config.auth[k] = (config.auth[k] || []).map(i => new RegExp(String.raw`^${(i !== "/login" ? i.replace(/\.\*/g, "*").replace(/\*/g, ".*") : "$")}`, "i"));
	});

	if (async) {
		iterate(config.auth, (v, k) => {
			if (v.enabled) {
				authMap[String.raw`${k}_uri`] = String.raw`/auth/${k}`;
				config.auth.protect.push(new RegExp(String.raw`^/auth/${k}`));
			}
		});
	}
...
function bootstrap (obj) {
	...
	if (obj.config.websocket.enabled && uws !== void 0) {
		...
		obj.log(String.raw`Started WebSocket server on ${obj.config.hostname}:${obj.config.websocket.options.port}`, "debug");
	}
	...
	if (obj.config.coap.enabled && coap !== void 0) {
		...
		obj.log(String.raw`Started COAP (${obj.config.coap.options.type}) server on ${obj.config.hostname}:${obj.config.coap.options.port}`, "debug");
	}
		...
		if (method === "coap") {
			if (obj.coap !== null) {
				iterate(routes, (fn, event) => {
					obj.log(String.raw`COAP event handler: '${event}'`, "debug");
...
function hypermedia (server, req, rep, headers) {
		...
		if (keys.length === 0) {
			...
			iterate(keys, i => {
				...
				if (regex.id.test(i) || regex.hypermedia.test(i)) {
					...
					uri = regex.scheme.test(obj[i]) ? obj[i] : String.raw`/${lcollection}/${obj[i]}`;
	...
	if (Array.isArray(rep.data)) {
		if (req.method === "GET" && (rep.status >= 200 && rep.status <= 206)) {
			...
			if (nth > 1) {
				...
				root += String.raw`?${Reflect.ownKeys(query).map(i => i + "=" + encodeURIComponent(query[i])).join("&")}`;
				...
				if (page - 1 > 1 && page <= nth) {
					rep.links.push({uri: root.replace("page=0", String.raw`page=${(page - 1)}`), rel: "prev"});
				}

				if (page + 1 < nth) {
					rep.links.push({uri: root.replace("page=0", String.raw`page=${(page + 1)}`), rel: "next"});
				}

				if (nth > 0 && page !== nth) {
					rep.links.push({uri: root.replace("page=0", String.raw`page=${nth}`), rel: "last"});
				}
			} else {
				root += String.raw`?${Reflect.ownKeys(query).map(i => i + "=" + encodeURIComponent(query[i])).join("&")}`;
			}
		...
		iterate(rep.data, i => {
			if (i instanceof Object) {
				...
				if (li !== collection) {
					const uri = li.indexOf("//") > -1 || li.indexOf("/") === 0 ? li : String.raw`${collection}/${li}`.replace(/^\/\//, "/");
	...
	if (rep.links.length > 0) {
		...
		headers.link = keysort(rep.links, "rel, uri").map(i => String.raw`<${i.uri}>; rel="${i.rel}"`).join(", ");
	}
