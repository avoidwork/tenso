...
function bootstrap(obj, config) {
	...
	if (config.websocket.enabled) {
		obj.websocket = new lws.Server({
			port: config.websocket.port,
			keepAliveInterval: config.websocket.keepAliveInterval
		});
		...
		if (method === "socket") {
			if (obj.websocket) {
				iterate(routes, function (arg, event) {
					...
					(_obj$websocket = obj.websocket).on.apply(_obj$websocket, [event].concat(_toConsumableArray(function (args) {
						arg.apply(undefined, _toConsumableArray(args).concat([obj.websocket, obj]));
					})));
				});
			}
		} else {
