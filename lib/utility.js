...
function auth (obj, config) {
	...
	let keys, sesh, fnCookie, fnSession, luscaCsp, luscaCsrf, luscaXframe, luscaP3p, luscaHsts, luscaXssProtection, passportAuth,
	...
	array.each(Object.keys(authMap), i => authUris.push(i));
	...
	if (config.auth.jwt.enabled) {
		(function () {
			...
			array.each(["algorithms", "audience", "issuer"], i => {
			...
			passport.use(new JWTStrategy(opts, (token, done) => {
				config.auth.jwt.auth(token.sub, (err, user) => {
					if (err) {
						done(err);
					} else {
						done(null, user);
