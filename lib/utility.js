...
function bootstrap (obj) {
	...
	obj.router.onsend = (req, res, body, status, headers) => {
	...
	if (obj.config.static !== "") {
		...
		obj.config.routes.get[`${spath}(/.*)?`] = (req, res) => req.server.router.serve(req, res, req.url.replace(new RegExp(`${spath.replace(/\//g, "\\/")}(\\/)?`), ""), path.join(__dirname, "..", "www", obj.config.static));
	}
