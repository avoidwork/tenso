		...
		if (keys.length === 0) {
			...
			each(keys, i => {
				...
				if (regex.id.test(i) === true || regex.hypermedia.test(i) === true) {
					...
					uri = regex.scheme.test(obj[i]) ? obj[i] : `/${lcollection.replace(/\s/g, "%20")}/${obj[i].toString().replace(/\s/g, "%20")}`;
...
function serialize (req, arg, status = 200) {
	let format = req.server.config.mimeType,
		accepts = explode(req.parsed.searchParams.get("format") || req.headers.accept || format, ","),
