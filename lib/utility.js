...
function bootstrap (obj) {
	...
	obj.router.removeListener("error", obj.router.error);
	obj.router.addListener("error", obj.error);
	obj.router.addListener("finish", obj.finish.bind(obj));
	...
	if (obj.config.etags.enabled) {
		...
		obj.always(obj.etags.middleware).ignore(obj.etags.middleware);
	}
