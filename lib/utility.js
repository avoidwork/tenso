...
function auth (obj, config) {
	...
	function init (authentication = false, sess = false) {
		if (authentication) {
			passportInit = passport.initialize();
			obj.router.always(passportInit).blacklist(passportInit);

			if (sess === true) {
				passportSession = passport.session();
				obj.router.always(passportSession).blacklist(passportSession);
			}
		}
	}
	...
	obj.router.always(middleware.parse).blacklist(middleware.parse);
	...
	if (stateless === true && stateful === false) {
		...
		if (authUris.length > 0) {
			...
			(function () {
				...
				r = r.replace(/\|$/, "") + ")).*$";
				obj.router.always(r, middleware.guard).blacklist(middleware.guard);
			}());
	...
	if (config.auth.basic.enabled === true) {
		(function () {
			...
			if (async === true || config.auth.local.enabled === true) {
				...
				obj.router.always(passportAuth).blacklist(passportAuth);
			}
		}());
	}
